...
\begin{frame}{Overview and Objectives}
  ...
  \begin{minipage}[t]{0.48\textwidth}
    \begin{tcolorbox}[stressbox, title=Configurations studied]
      ...
      \begin{itemize}\setlength\itemsep{0.1pt}
        \item \textbf{Full aperture:} circular array
        \item \textbf{Partial aperture:} linear array
        \item \textbf{Broadband:} time-dependent signal
        \item \textbf{Noisy:} additive Gaussian noise
      \end{itemize} 
    \end{tcolorbox}
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.48\textwidth}
    \begin{tcolorbox}[stressbox, title=Workflow]
      \footnotesize
      \begin{enumerate}\setlength\itemsep{0.1pt}
        \item Green's function \& Born data
        \item Implement RT, KM, MUSIC
        \item Analyze results
        \item Stability under noise (Monte Carlo)
      \end{enumerate}
    \end{tcolorbox}
  \end{minipage}
  ...
  \begin{tikzpicture}[>=Stealth, font=\scriptsize, scale=0.7]
    ...
    \node[font=\tiny, alertorange, right] at (0.45,0.2) {reflector $\bm{x}_{\mathrm{ref}}$};
    % Waves
...
\begin{frame}{Imaging Functionals: RT, KM and MUSIC}
  ...
  \begin{tcolorbox}[keybox, title=Reverse-Time (RT) Migration, top=2pt, bottom=2pt]
    \footnotesize
    Back-propagate received signals using the \emph{exact} Green's function:
    \vspace{-0.15cm}
  ...
  \begin{tcolorbox}[stressbox, title=Kirchhoff Migration (KM), top=2pt, bottom=2pt]
    \footnotesize
    Approximate $\hat{G}_0 \approx e^{i\omega\mathcal{T}}$ (travel-time only, drop amplitude):
    \vspace{-0.15cm}
  ...
  \begin{tcolorbox}[resultbox, title=MUSIC (Signal-Subspace Projection), top=2pt, bottom=2pt]
    \footnotesize
    Exploit the rank-1 structure of $\hat{U}$; project onto the signal subspace:
    \vspace{-0.15cm}
    $$\mathcal{I}_{MU}(\bm{x}) = |\langle \hat{g}(\omega, \bm{x}),\, v_1 \rangle|^2$$
    \vspace{-0.3cm}

    {\scriptsize where $\hat{g}(\omega,\bm{x}) = (\hat{G}_0(\omega, \bm{x}, x_r))_{r=1,\dots,N}$ and $v_1$ is the first singular vector of $\hat{U}$.}
  \end{tcolorbox}
\end{frame}
  ...
  \begin{column}{0.38\textwidth}
    \begin{tcolorbox}[keybox, title=Observations]
      ...
      \begin{itemize}\setlength\itemsep{2pt}
        \item RT and KM images are very similar; minor discrepancies from the KM approximation.
...
\begin{frame}{Partial Aperture -- Influence of Array Length $R_0$}
  ...
  \begin{column}{0.44\textwidth}
    ...
    \begin{tcolorbox}[stressbox, title=Key insight]
      \scriptsize As $R_0$ decreases, the focal spot \textbf{widens} in the cross-range direction, consistent with the theoretical $\operatorname{sinc}^2$ prediction.
    \end{tcolorbox}
  \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Stability -- Full Aperture under Noise}
  ...
  \begin{tcolorbox}[resultbox, title=Result]
    \footnotesize RT and KM both \textbf{degrade gracefully} as $\sigma$ increases. Coherent integration over many transducer pairs provides a natural averaging effect that partially mitigates noise.
  \end{tcolorbox}
\end{frame}
...
\begin{frame}{Stability -- Monte Carlo Analysis: RT vs.\ MUSIC}
  ...
  \begin{column}{0.50\textwidth}
    \begin{tcolorbox}[resultbox, title=RT: graceful degradation]
    ...
    \vspace{-0.3cm}
    \begin{tcolorbox}[warnbox, title=MUSIC: fragile super-resolution]
...
\begin{frame}{Conclusion}
  ...
  \begin{tcolorbox}[keybox, title=Summary of Findings, top=2pt, bottom=2pt]
    \begin{itemize}\setlength\itemsep{1pt}
      \item[\textcolor{greenok}{\textbf{\checkmark}}] \textbf{Full aperture:} RT and KM agree closely; both approach theoretical $J_0^2$ spot as $R_0,N\!\to\!\infty$.
      \item[\textcolor{greenok}{\textbf{\checkmark}}] \textbf{Partial aperture:} linear geometry $\Rightarrow$ mirror artifact in range; cross-range resolution $r_c\!=\!\lambda|x_{\mathrm{ref}}|/R_0$. MUSIC provides sharper image.
      \item[\textcolor{greenok}{\textbf{\checkmark}}] \textbf{Broadband:} increased bandwidth drastically improves range resolution ($r_l \!\propto\! 1/B$), consistent with $\operatorname{sinc}$ theory.
      \item[\textcolor{greenok}{\textbf{\checkmark}}] \textbf{Noise:} RT degrades gracefully; MUSIC is precision-first but collapses beyond a noise threshold.
    \end{itemize}
  \end{tcolorbox}
